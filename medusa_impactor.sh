...
# Replace the final test build section with this improved version
echo "🧪 Running initial test build..."
if cargo build 2>/tmp/build_errors.log; then
    echo "✅ Initial build successful!"
else
    echo "⚠️ Initial build completed with warnings (this is normal)."
    echo "   The bindings have been generated successfully."
    echo "   You can proceed with the next steps."
fi

echo "💡 Note: Some warnings during first build are expected and can be ignored."
}
